...
pv_ref = 3
pv_data = pd.read_csv('./Dataset/H2_W_resampled_15min.csv', sep = ',').to_numpy()

pv_data = pv_data[:4*24, 3] # extract 1 day data
#pv_data = pv_data.astype(str)
#pv_data = np.char.replace(pv_data, ',', '.')
pv_data = pv_data.astype(float)
pv_data = np.mean(pv_data.reshape(-1, 4), axis=1) # average to have 1-hour resolution
...
# load
load_fix_ref = 3
load_fix_data = pd.read_csv('./Dataset/Consumer_power.csv', sep = ',').to_numpy()
load_fix_data = load_fix_data[:4*24, 4]
#load_fix_data = load_fix_data.astype(str)
#load_fix_data = np.char.replace(load_fix_data, ',', '.')
load_fix_data = load_fix_data.astype(float)
load_fix_data = np.mean(load_fix_data.reshape(-1, 4), axis=1) # average
