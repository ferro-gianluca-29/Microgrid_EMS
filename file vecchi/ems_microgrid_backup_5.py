...
input_features, _ = create_windows(price.values[:500], lookback, forecast_horizon)
...
_, target_price = create_windows(price.values[:500], lookback, forecast_horizon)
...
_, P_r_windows = create_windows(P_r.values, lookback, forecast_horizon)
_, P_l_windows = create_windows(P_l.values, lookback, forecast_horizon)
...
output_dim = target_price.shape[-1]
hidden_dim = 48
# init for test
