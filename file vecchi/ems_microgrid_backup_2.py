...
class EnergyManagementSystem(optModel):
    def __init__(self, P_r_data, P_l_data, price_forecast, T=24):
        """
        Args:
    ...
    def setObj(self, c):
        ...
        self._model.obj = Objective(expr=sum(-converted_prices[t] * self._model.P_g[t] for t in range(len(c))), sense=minimize)
    ...
    def solve(self):
        ...
        if results.solver.status == 'ok' and results.solver.termination_condition == 'optimal':
            ...
            self.P_s_values = np.array([value(self._model.P_s[t]) for t in range(self.T)])
    ...
    def plot_variables(self):
        ...
        if not hasattr(self, 'P_g_values') or not hasattr(self, 'P_s_values') or not hasattr(self, 's_values'):
        ...
        # Plot di P_s
        plt.plot(time, self.P_s_values, label="Potenza Scambiata (P_s)", marker='o')

        # Plot di s
        plt.plot(time_s, self.s_values, label="Stato di Carica (s)", marker='s')  # Usa il tempo corretto

        # Plot di P_g
        plt.plot(time, self.P_g_values, label="Potenza alla Rete (P_g)", marker='^')
...
df = pd.DataFrame({
    'PV Data': pv_data.values[:1000],
    'Load Data': load_data.values[:1000],
    'Price Data': price_data.values[:1000]
})
...
model = EnergyManagementSystem(P_r, P_l, price, T = forecast_horizon)
...
# prova: ottimizzazione su una singola finestra

model.setObj(price_windows_test[6])
