...
class EnergyManagementSystem(optModel):
    ...
    def setObj(self, c):
        ...
        self._model.obj = Objective(expr=sum(-self.converted_prices[t] / 1000 * (self._model.P_r[t] - self._model.P_l[t] -  
...
df = pd.DataFrame({
    'PV Data': pv_data.values[:500],
    'Load Data': load_data.values[:500],
    'Price Data': price_data.values[:500]
})
...
model = EnergyManagementSystem(P_r[24:48], P_l[24:48], price[24:48], T = forecast_horizon) 

#🎈🎈🎈🎈🎈🎈 SOLUZIONE PER UNA FINESTRA
# Imposta l'obiettivo di ottimizzazione
model.setObj(price_windows[1])
sol, obj = model.solve()
