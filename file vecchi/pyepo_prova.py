...
class myModel(optModel):
        ...
        def _getModel(self):
            ...
            # Parametri
            m.P_prod = Param(m.T, initialize={t: self.P_prod[t] for t in m.T},mutable=True)
            m.P_load = Param(m.T, initialize={t: self.P_load[t] for t in m.T}, mutable=True)
        ...
        def setObj(self, P_prod=None, P_load=None):
            model = self._model
            if hasattr(model, 'obj'):
                model.del_component('obj')  # Rimuove l'obiettivo esistente se presente

            # Aggiornamento dei parametri di produzione e consumo, controllando se sono forniti
            if P_prod is not None:
                for t in model.T:  # Assumendo T come l'insieme degli indici temporali
                    model.P_prod[t].value = P_prod[t-1]  # Aggiornamento del valore

            if P_load is not None:
                for t in model.T:
                    model.P_load[t].value = P_load[t-1]  # Aggiornamento del valore

            # Definizione della nuova funzione obiettivo
            model.obj = Objective(
...
if __name__ == '__main__':
    ...
    optmodel = myModel(pv_data, load_fix_data)

    optmodel.setObj(P_prod=pv_data[:24], P_load=load_fix_data[:24])
    ...
    train_dataset = optDataset(optmodel, x_train, c_train)
    test_dataset = optDataset(optmodel, x_test, c_test)
