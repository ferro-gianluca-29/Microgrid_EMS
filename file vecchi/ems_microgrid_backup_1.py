...
class EnergyManagementSystem(optModel):
    def __init__(self, P_r_data, P_l_data, price_forecast):
        """
        Args:
            ...
            price_forecast (list[float]): Previsione dei prezzi dell'elettricità per l'orizzonte T.
    ...
    def _getModel(self):
        ...
        # Variabili
        m.P_s = Var(range(self.T), bounds=(-self.P_s_max, self.P_s_max))
        m.s = Var(range(self.T + 1), bounds=(self.s_min, self.s_max))
        m.P_g = Var(range(self.T), within=Reals)

        # Parametri

        # Conversione del prezzo da euro/MWh a euro/kWh nel setup del modello
        m.price = Param(range(self.T), initialize={t: self.price_forecast[t] / 1000 for t in range(self.T)})
        ...
        m.state_transition = Constraint(range(self.T), rule=lambda model, t: model.s[t + 1] == 
                                   (1 - self.sigma) * model.s[t] + self.eta_c * self.T_s * model.P_s[t])
        
        m.power_balance = Constraint(range(self.T), rule=lambda model, t: model.P_g[t] == 
                                     model.P_r[t] - model.P_l[t] - model.P_s[t])

        # Variables for optimization
        variables = {'P_s': m.P_s, 's': m.s, 'P_g': m.P_g}
        return m, variables
    ...
    def setObj(self, c):
        ...
        self._model.obj = Objective(expr=sum(-converted_prices[t] * self._model.P_g[t] for t in range(self.T)), sense=minimize)
    ...
    def solve(self):
        ...
        if results.solver.status == 'ok' and results.solver.termination_condition == 'optimal':
            optimal_solution = {
                'P_s': [value(self._model.P_s[t]) for t in range(self.T)],
                's': [value(self._model.s[t]) for t in range(self.T + 1)],
                'P_g': [value(self._model.P_g[t]) for t in range(self.T)]
            }
            objective_value = value(self._model.obj)
    ...
    def plot_variables(self):
        """
        Genera un grafico delle variabili decisionali in funzione del tempo.
        """
        # Soluzione ottimale
        ...
        time = list(range(self.T))  # Converte il range in una lista
        time_s = list(range(self.T + 1))  # Per la variabile 's' che ha un punto in più

        # Plot delle variabili
        plt.figure(figsize=(12, 6))

        # Plot di P_s
        plt.plot(time, P_s, label="Potenza Scambiata (P_s)", marker='o')

        # Plot di s
        plt.plot(time_s, s, label="Stato di Carica (s)", marker='s')  # Usa il tempo corretto

        # Plot di P_g
        plt.plot(time, P_g, label="Potenza alla Rete (P_g)", marker='^')
...
# Dato che i metodi assumono di ricevere liste o array, utilizziamo .squeeze() per convertire da DataFrame a Series
df = pd.DataFrame({
...
print("Dimensioni delle features:", features.shape)        # Dovrebbe essere (num_windows, 24, 3)
print("Dimensioni delle price_windows:", price_windows.shape)  # Dovrebbe essere (num_windows, 12)


model = EnergyManagementSystem(P_r, P_l, price)


# Imposta l'obiettivo di ottimizzazione
#model.setObj(price)

optimal_solution, objective_value = model.solve()

model.setObj(price_windows[0])

model.plot_variables()

print("Soluzione Ottimale:", optimal_solution)
print("Valore della Funzione Obiettivo:", objective_value)
...
dataset_train = optDataset(model, x_train, c_train)
# get test data set
dataset_test = optDataset(model, x_test, c_test)
